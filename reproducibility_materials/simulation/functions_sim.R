...
# E means number of covariate plus the intercept
# This version adds extra smoothing on Z
create_set_nonlinear <- function(n, nz, E, R, Q, sig2X, sig2Z, sig2theta, tau2r, lambda0, lambda1, lambda2, betas, B1, B2, rho1, rho2, n1, bXZ, bw, k, b){
  library(fields)
  # AR1 structures
  ar1_e <- ar1_cor(E-1, rho1)
  ar1_r <- ar1_cor(R, rho2)
  ...
  UB2 <- smoothZ%*%B2 
  UB2_transform <- sign(UB2)*abs(UB2)^k
  # UB2_cube <- UB2^3
  # cube <- .8*(UB2_cube) 
  # theta <- .1*(cube + M2%*%t(chol(ar1_r)))
  ck = b/sd(UB2_transform)
  theta <- ck*UB2_transform + M2%*%t(chol(ar1_r))
  # estar nxR
  e <- matrix(0, nrow = n, ncol = R)
  for (i in 1:R){
    e[,i] <- rnorm(n, 0, sqrt(tau2r[i]))
...
sim_func <- function(n, nz, E, R, L, K, Qmat, sig2X, sig2Z, sig2theta, tau2r, lambdaZ, lambda1, lambda2, betas, B1, B2, rho1, rho2, iters, burn, num1, num2, n1, bXZ, bw, i){
  set.seed(i)
  library(MASS)
  library(plyr)
  library(splines2)
  ...
  test[[2]] <- MCMC_model3_2(Ystar, Xstar, Q = Qmat, p = E, n = n, r = R, q = R, iters = iters, burn = burn, 1, n)
  # one-step spatial plus
  test[[3]] <- MCMC_model3_2(Ystar, Xstar, Q = Qmat, p = E, n = n, r = R, q = R, iters = iters, burn = burn, num1, num2)
  test[[4]] <- MCMC_model3_2(Ystar, Xstar, Q = Qmat, p = E, n = n, r = R, q = R, iters = iters, burn = burn, num1, num2/2)
  for (k in 1:E){
    for (l in 1:R){
      for (m in 2:4){
  ...
  tol_time <- 0
  for (r in 1:R){
    test[[r+4]] <- MCMC_model4_9_v2(Ystar[,r], Xstar, Qmat = Qmat, E = E, n = n, L, K, B = B, iters = iters, burn = burn)
    tol_time <- tol_time + test[[r+4]]$time[3]
  }
  for (j in 1:n){
    for (k in 1:E){
      for (l in 1:R){
        for (m in 5:5){
          results[j,k,l,1,m] <- mean(test[[(l+4)]]$beta[j,k,])
          results[j,k,l,2,m] <- quantile(test[[(l+4)]]$beta[j,k,], probs = c(0.025))
          results[j,k,l,3,m] <- quantile(test[[(l+4)]]$beta[j,k,], probs = c(0.5))
          results[j,k,l,4,m] <- quantile(test[[(l+4)]]$beta[j,k,], probs = c(0.975))
          results[j,k,l,5,m] <- sd(test[[(l+4)]]$beta[j,k,])
          results[j,k,l,6,m] <- tol_time
          results[j,k,l,7,m] <- effectiveSize(test[[(l+4)]]$beta[j,k,])
        }
      }
    }
  }
...
# 8 competing models
# no standard normal prior
sim_func_nonlinear <- function(n, nz, E, R, L, K, Qmat, sig2X, sig2Z, sig2theta, tau2r, lambdaZ, lambda1, lambda2, betas, B1, B2, rho1, rho2, iters, burn, num1, num2, n1, bXZ, bw, k, b, i){
  set.seed(i)
  library(MASS)
  library(plyr)
  library(splines2)
  ...
  # B20 <- bSpline(W,df = 20, intercept = T)
  # B30 <- bSpline(W,df = 30, intercept = T)
  set <- create_set_nonlinear(n, nz, E, R, Qmat, sig2X, sig2Z, sig2theta, tau2r, lambdaZ, lambda1, lambda2, betas, B1, B2, rho1, rho2, n1, bXZ, bw, k, b)
  Ystar <- t(Gamma)%*%set$Y
  Xstar <- t(Gamma)%*%set$X
  # mean, first, median, third, sd, time
  results <- array(0, c(n, E, R, 7, 2))
  test <- list()
  # full model, HC prior
  test[[1]] <- MCMC_model4_8_v7(Ystar, Xstar, Qmat = Qmat, E = E, n = n, R = R, Q = R, L, K, B = B, iters = iters, burn = burn, full_result = T, W=W)
  # test[[2]] <- MCMC_model4_8_v7(Ystar, Xstar, Qmat = Qmat, E = E, n = n, R = R, Q = R, L=20, K, B = B20, iters = iters, burn = burn, full_result = T, W=W)
...
metrics_over_all3 <- function(sim, true, M){
  E <- nrow(true) 
  R <- ncol(true) 
  MSE1 <- bias1 <- cov1 <- sd1 <- length1 <- array(0, c(M,E,R))
  MSE1[,1,] <- bias1[,1,] <- cov1[,1,] <- sd1[,1,] <- length1[,1,] <- NA
  for (m in 1:M){
    for (i in 2:E){
      for (j in 1:R){
  ...
  sd_ave1 <- rowMeans(sd1, na.rm = TRUE, dims = 1)
  length_ave1 <- rowMeans(length1, na.rm = TRUE, dims = 1)
  out <- list(MSE_ave1 = MSE_ave1, bias_ave1 = bias_ave1, cov_ave1 = cov_ave1, sd_ave1 = sd_ave1, length_ave1 = length_ave1)
  return(out)
}
