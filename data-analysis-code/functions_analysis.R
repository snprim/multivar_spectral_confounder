...
# lambda3 (lambdaRHC) is the same as tauHC
# allow for covariates (Z) and treatments (X)
# save theta (AU), alpha, and residual
MCMC_model4_8_4_XZ <- function(Y, X, Z, Qmat, E, n, R, Q, L, K, B, iters = 1000, burn = 50, W, thin = 2){
  tik <- proc.time()
  # Aindex keeps track of the number of entries in each row of A to be updated
  Aindex <- rep(0, R)
  for (r in 1:R){
  ...
  keepers_Te <- array(0, c(E, K, iters_thin))
  keepers_Tr <- array(0, c(R, K, iters_thin))
  keepers_AU <- array(0, c(n, R, iters_thin))
  ...
  estimate <- find_estimate(X_tensor, B, Tl, Te, Tr, n, R)
  alpha <- matrix(0, nrow = p, ncol = R)
  keepers_alpha <- array(0, c(p, R, iters_thin))
  keepers_residual <- array(0, c(n, R, iters_thin))
  pb <- txtProgressBar(min = 2, max = iters, style = 3)
  for (iter in 2:iters){
    # print(paste0("This is the ", iter, "th iterations."))
    # update tensor margins for beta
    ...
    if (iter > burn){
      if ((iter-burn)%%thin == 0){
        ...
        keepers_Te[,,(iter-burn)%/%thin] <- Te
        keepers_Tr[,,(iter-burn)%/%thin] <- Tr
        keepers_AU[,,(iter-burn)%/%thin] <- AU
        keepers_alpha[,,(iter-burn)%/%thin] <- alpha
        keepers_residual[,,(iter-burn)%/%thin] <- Y-AU-Zalpha-estimate
      }
    }
  ...
  tok <- proc.time()
  out <- list(Tl = keepers_Tl, Te = keepers_Te, Tr = keepers_Tr, theta = keepers_AU, alpha = keepers_alpha, residual = keepers_residual, acc_rate = acc/att, time = tok - tik, MH = MH)
  return(out)
}
...
MCMC_model3_2_XZ <- function(Y, X, Z, Qmat, p = 5, n, R, Q, iters = 1000, burn = 50, num1, num2, W, thin=2){
  ...
  pb <- txtProgressBar(min = 2, max = iters, style = 3)
  for (iter in 2:iters){
    # if (iter %% 100 == 0) print(paste0("This is the ", iter, "th iterations."))
    Zalpha <- Z%*%alpha
    for (i in 1:R){
      # update beta_r
